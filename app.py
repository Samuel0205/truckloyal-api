...
def _send_reset_email(to_email: str, reset_url: str, user_type: str, name: str) -> bool:
    ...
    if not resend_key:
        ...
        print(f"[PASSWORD RESET] No RESEND_API_KEY set. Reset URL: {reset_url}")
        return False
    ...
    <body style="font-family:Arial,sans-serif;background:#FFF8F0;padding:32px;margin:0">
      <div style="max-width:480px;margin:0 auto;background:white;border-radius:16px;padding:32px;box-shadow:0 4px 20px rgba(0,0,0,.08)">
        ...
        <p style="color:#ccc;font-size:11px;text-align:center">
          Food Truck Rewards · support@foodtruckrewards.app<br>
...
def forgot_password():
    ...
    try:
        ...
        row = sb.table(table).select(f"id, email, {name_field}").eq("email", email).execute().data
...
def reset_password():
    ...
    now = datetime.utcnow()
    tokens = sb.table("password_reset_tokens").select("*").eq("email", email).eq("user_type", user_type).eq("used", False).execute().data
    ...
    for t in tokens:
        # Check expiry first
        exp = datetime.fromisoformat(t["expires_at"].replace("Z", ""))
        if exp < now:
            continue
